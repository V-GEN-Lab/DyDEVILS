# This script contains a modified version of the funtion evaluate_candidate (line 75). The modification includes to only evaluate nodes 
# as possible lineage canditades considering values UFBoot ≥ 90% 

# Docstring from original authors:
'''
    ...
    for node in rbfs:
        if node.is_leaf():
            ...
            if node.id not in ignore:
                sum_and_count_dict[node.id] = (len(node.mutations), 1, node.confidence)
        else:
            total_count = 0
            total_sum = 0
            ...
            if total_count > 0:
                ...
                sum_and_count_dict[node.id] = (total_sum + len(node.mutations) * total_count, total_count, node.confidence)
...
def evaluate_candidate(a, nid, sum_and_counts, dist_to_root, minimum_size=0,minimum_distinction=0):
    ...
    """
    node_sum, node_count, bootstrap = sum_and_counts.get(nid,[0,0,0])
    if node_count <= minimum_size:
        return 0
    if node_sum == 0 or node_count <= 0:
        return 0
    if bootstrap < 90: # line added
        return 0
...
class Tree:
    ...
    def __loader(self, jd, cnode, use_aa=False, gene=None):
        ...
        #print(jd['node_attrs'])
        try:
            confidence = jd['node_attrs']['confidence']['value']
        except KeyError:
            #print(f"WARNING: this is the root node",file=sys.stderr)
            confidence = 0
        #cnode.confidence = jd['node_attrs']['confidence']['value']
        cnode.confidence = confidence
        #print(cnode.confidence)
...
def pipeline(ijd, ojson, floor=0, size=0, distinction=0, cutoff=1, missense=False, gene=None, maxlevels=0, labels=None, reportf=None):
    ...
    # t is the tree structure, with node ids
        ...
        for ann,nid in outer_annotes.items():
            ...
            while True:
                ...
                if len(labeled) >= leaf_count * cutoff:
                    break
                serial += 1
                print(f"Annotation {newname} generated for node {best_node.id}. With GRI = {best_score}")
